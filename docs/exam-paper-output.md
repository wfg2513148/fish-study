...
4. 选择题按题目数量约占 60%。统计口径：带选项块的题算选择题，问答、填空、写作、实验探究不算选择题。
5. 涉及绘图的题必须使用 `gpt-image-2` 生成正式配图，不能用占位图、CSS 示意图或临时 SVG 冒充。
6. 科学卷需要大量图文题，图片以黑白教材/试卷风格线稿为主，便于打印。
7. 图片不能过窄。数学图 A、英语地图、科学图示都要保持足够阅读宽度。
8. 数学解答题答题框要足够大，避免学生写不下完整过程。
9. 最终交付 PDF，不交付依赖外链图片的 HTML；PDF 内必须嵌入图片，避免换电脑或手机后图片丢失。
10. PDF 底部必须有页码，格式为 `当前页/总页数`，例如 `1/5`、`1/8`。
11. 科学卷图片多，必须控制 PDF 体积，避免手机打开超时。
12. 交付到 Codex 会话时，使用中文 PDF 文件名和 Markdown 绝对路径文件链接，例如 `[七年级下册数学模拟试卷.pdf](/Users/kwang/fish-study/outputs/codex-session-files/七年级下册数学模拟试卷.pdf)`。
...
| 学科 | 选择题比例 | 图片数 | 说明 |
|---|---:|---:|---|
| 数学 | 15/25 = 60% | 5 处图片引用 | 解答题答题框已加大，图 A 宽度已优化 |
| 科学 | 18/30 = 60% | 20 处图片引用 | 使用压缩后本地图片，手机打开更稳 |
| 英语 | 27/45 = 60% | 1 处图片引用 | 地图宽度已优化 |
