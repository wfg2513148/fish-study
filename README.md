...
在 Codex Desktop 里上传一张或多张照片，然后只说：

```text
帮我生成错题知识点和测试题
```

Codex 应使用 `.codex/skills/fish-study-photo-workflow/SKILL.md`，自动读取知识库范围、识别照片所属学科和红黄蓝标注，再按学科分别生成训练卷、答案页和知识点讲解。家长不需要手写 JSON。
...
学生只看训练卷。答案页给家长批改，不要提前给孩子。一次上传多科照片时，优先打印对应学科的训练卷。
...
每张照片写入 source_photos，并给每个分析组合写入 source_photo_ids。
看不清、学科不确定或知识点不确定的题，不要猜，放入 uncertain_items。
```
